...
railNote: "개인 · 2026.01"
results:
  - { v: "0.960 → 0.977", k: "리더보드 ROC-AUC · 누출 교정 + 이종 증류 후" }
  - { v: "18위 상당", k: "Kaggle 리더보드" }
  - { v: "0.980", k: "누출 상태의 검증 점수 · 같은 시점 LB 0.960" }
fixes:
...
Kaggle FGVC7 Plant Pathology 대회 — 식물 잎 이미지에서 병해(healthy / multiple_diseases / rust / scab)를 분류합니다. 검증 성능(0.980)과 실제 리더보드 점수(0.960)의 큰 갭은, 학습·평가 이미지가 겹쳐 검증 점수가 실제보다 부풀려지는 데이터 누출의 신호였습니다.
...
3. **이종 지식 증류**<br>큰 모델 ConvNeXt(Teacher)의 판단을 가벼운 ResNeSt(Student)에 이식 · 온도 T=1.25
4. **Top-k(3) 앙상블**<br>성적 상위 3개 모델의 예측을 평균해 최종 제출
...
1위 솔루션을 코드 없이 논문·설명만으로 분석해 직접 구현했습니다. ConvNeXt를 Teacher, ResNeSt를 Student로 설정한 이종 증류는 큰 모델(Teacher)의 판단 분포(soft label)를 가벼운 모델(Student)이 따라 배우는 구조이며, 온도(T=1.25)는 그 판단을 얼마나 부드럽게 전달할지 조절하는 값입니다. timm 라이브러리로 백본을 빠르게 교체 실험했습니다.
...
- 최종 ROC-AUC 0.977 — 데이터 누출 교정 + 지식 증류 적용 후 리더보드 점수
- Kaggle 리더보드 18위 상당
