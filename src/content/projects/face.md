...
**나이 회귀(본인 담당):** InceptionResNetv1 마지막 블록 미세조정. 60대 이상 이미지 오버샘플링. 데이터 분포(0~19세 ~75% 편중)가 집계 지표를 왜곡해 고연령 실패를 은폐하는 문제를 발견 → 연령대별 MAE·CS(5)(예측이 실제 나이 ±5세 이내인 비율) 분리 지표 도입.
...
- 오버샘플링은 정보량을 늘리지 않는다 — 불균형 회귀 기법(LDS/FDS — 데이터가 적은 구간을 분포 기준으로 보정)·손실 재가중이 올바른 접근.
- 팀장으로서 역할 분리(나이/성별/감정)와 통합 파이프라인 설계를 병행했다.

## Next Improvements

40대+ 데이터 수집·증강 · 비율유지 리사이즈 전처리 · 불균형 인지 지표(AAR — 연령대별 오차를 고르게 평가) 도입.
